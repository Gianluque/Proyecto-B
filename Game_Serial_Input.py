...
def serialGet(port,flushtimer):
    ...
    dig1=int(d1)
    dig2=int(d2)
    jump=int(ch1,2)*3/65535
    shoot=int(ch2,2)*3/65535

    return(jump,shoot,dig1,dig2,flushtimer)
...
m#usic = pygame.mixer.music.load('music.wav')
...
while True:
    ...
    while run :
        ...
        (shoot,jump,enter,select,auxtimer)=serialGet(puerto,auxtimer)
        ...
        if len(stones) < 1 and stoneLoop == 0:
    ...
    while run:
        ...
        Restart = font2.render('RESTART:ENTER\nEXIT:SELECT ', 1, (255, 255, 255))
        (shoot,jump,enter,select,auxtimer)=serialGet(puerto,auxtimer)
